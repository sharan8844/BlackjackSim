var storedName = JSON.parse(localStorage.getItem("arr"));
var players = localStorage.getItem("playercount");
let n = players;
...
function finalcall() {
  for (zzz = 0; zzz < n; zzz++) {
    if (score[zzz] == scoredealer && scoredealer <= 21) {
      ...
      document.querySelector(
        `#amount${zzz}`
      ).innerText = `Amount Left:${amount[zzz]}`;
    } else if (
      score[zzz] > 21 ||
      (score[zzz] < scoredealer && scoredealer <= 21)
    ) {
      ...
      (score[zzz] <= 21 && scoredealer > 21) ||
      (score[zzz] <= 21 && score[zzz] > scoredealer)
    ) {
  ...
  document.getElementById(
    "reload"
  ).innerHTML = `               <button onclick="window.location.reload();" type="button" class="btn btn-outline-success align-center mr-5 mt-5 drawbutton" style="margin-left: 22%;width:8%;padding:2%;font-weight: bolder;">New game</button>
    `;
